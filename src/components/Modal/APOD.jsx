...
const APOD = () => {
  ...
  useEffect(() => {
    async function fetchAPOD() {
      try {
        console.log(BASE_URL + API_KEY + query)
        const response = await fetch(BASE_URL + '?api_key=' + API_KEY + query)
          ...
          <div className="modal-box max-w-3xl">
            ...
            <div className="modal-inner-btn mt-5">
              <div className="flex flex-col gap-3">
                <div
                  className="btn btn-outline w-full hover:bg-none"
                  id="date-picker"
                >
                  {isLoading ? (
                    ...
                    <div className="inner text-sm flex justify-around gap-5 px-[15%] md:px-[20%] lg:px-[25%] items-center">
                      ...
                      <input
                        type="date"
                        className="basis-2/3 text-center"
                        id="choose-date"
                        name="choose-date"
                        value={date}
                        min="1995-06-16"
                ...
                <div className="flex justify-between w-full gap-3">
                  {!isLoading ? (
                    <button
                      className="btn btn-outline basis-3/4"
                      onClick={() => handleDateChange(generateRandomDate())}
                    >
                      Get random date{' '}
                      <Icon
                        icon="ix:random"
                  ...
                  <button
                    className="btn btn-outline basis-1/4 shrink"
            ...
            {!isLoading ? (
              <>
                {apiData.media_type === 'image' ? (
                  ...
                  <div className="py-40">
                    <h2>
                      <em>Media not supported. Please pick another day.</em>
                    </h2>
                  </div>
                )}
